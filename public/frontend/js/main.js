...
$(document).on("click", ".action-btn-quick-view", function () {
    ...
    var redirecturl =
        $('meta[name="base_url"]').attr("content") + "/product_details/" + slug;
    var redirect_compareurl =
    ...
    $.ajax({
        ...
        success: function (response) {
            var reg_price =
                response.saleprice["tax_price"] +
                Number(response.productvariant.regular_price);
            ...
            $(".product_saleprice").html(
                Math.round(response.saleprice["sale_price"]).toFixed(2),
            );
            if (response.product.discount_type != "no_discount") {
                $(".product_regularprice").html(
                    "₹" + Math.round(reg_price).toFixed(2),
                );
            } else {
                $(".product_regularprice").html("");
            }
            $(".product_desciption").html(response.product.description);
            $(".viewdetails_redirect").html(
                '<small><a href=+"' +
                    redirecturl +
            ...
            $(".modal_decrement" + product_id).attr(
                "data-product_id",
                product_id,
            );
            $(".modal_increment" + product_id).attr(
                "data-product_id",
                product_id,
            );

            $(".modalcount_prod_qty").addClass(
                "modal_count_prod_qty" + product_id,
            );

            $(".hiddenmodalcount_prod_qty").addClass(
                "hidden_modalcount" + product_id,
            );
            $(".hidden_modalcount" + product_id).attr(
                "id",
                "count_prod_qty" + product_id,
            );

            $(".modal_count_prod_qty" + product_id).attr(
                "id",
                "count_prod_qty" + product_id,
            );
            $("#count_prod_qty" + product_id).attr(
                "data-product_id",
                product_id,
            );
            ...
            $(".rating_product" + product_id).attr(
                "style",
                "--value:" + response.relatedreview,
            );
...
$(document).on("click", ".cartremove", function () {
    ...
    if ($.fn.niceSelect && $("#sstate").length > 0) {
        $("#sstate").val("").niceSelect("update");
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#render_checkoutproduct_table").html(
                data.render_checkoutproduct_table,
            );
            $(".render_payment_info").html(data.payment_info);

            if (typeof productgstchange === "function") {
                productgstchange();
            }
            //cart_open();
...
$("#register_submit").click(function () {
    ...
    if (
        ...
        $("#register_phone_number").val() != "" &&
        $("#register_pwsd").val() != "" &&
        $("#register_pwsd").val().length >= 4
    ) {
        //$('.regname_err').text('');
        $(".regphone_err").text("");
        $(".regemail_err").text("");
...
$(document).on("click", ".qty-btn", function () {
    ...
    var val = $("#quantity" + product_id).val();
    var minqty = Number(0);
    var maxqty = Number($("#max_qty").val());
    ...
    if ($(this).data("type") == "inc") {
        if (val < maxqty) {
            console.log(maxqty);
            console.log(val);
            $("#quantity" + product_id).val(Number(val) + 1);
            $("#count_prod_qty" + product_id).val(Number(val) + 1);
        }
    } else {
        if (val > minqty) {
            $("#quantity" + product_id).val(Number(val) - 1);
...
$(document).on("click", "#review_submit", function () {
    ...
    if (
        $("#review_comment").val() != "" &&
        $("#review_customerphone").val() != "" &&
        $("#product_rating").val() != 0 &&
        $("#review_customername").val() != "" &&
        isemail_valid == true &&
        $("#review_customeremail").val()
    ) {
...
$(document).on("click", ".cart_rendercount", function () {
    ...
    if ($(this).data("type") == "inc") {
        $("#quantity" + product_id).val(Number(val));
        $("#count_prod_qty" + product_id).val(Number(val));
        $(".product_rate_" + product_id).html(
            Number(val) + " x " + Number(price),
        );
        render_cartlist(product_id, val);
    } else {
        $("#quantity" + product_id).val(Number(val));
        $("#count_prod_qty" + product_id).val(Number(val));
        $(".product_rate_" + product_id).html(
...
function render_cartlist(product_id, val) {
    var token = $('meta[name="csrf-token"]').attr("content");
...
$(document).on("click", "#update_userdetails", function () {
    var token = $('meta[name="csrf-token"]').attr("content");
    var path =
        $('meta[name="base_url"]').attr("content") + "/update_userdetails";
    var redirect =
        $('meta[name="base_url"]').attr("content") + "/customer/my_account";
    if (
        $("#user_name").val() != "" &&
        $("#user_email").val() != "" &&
        $("#user_previouspassword").val() != "" &&
        $("#user_new_password").val() != "" &&
        $("#user_confirm_newpassword").val() != "" &&
        $("#user_new_password").val() == $("#user_confirm_newpassword").val()
    ) {
