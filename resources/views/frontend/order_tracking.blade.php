                        ...
                        <hr class="mb-3"><br>
                         @if(!empty($order))
							<div class="mt-3 row justify-content-between">
								<div class="order-tracking {{($order->status=='Received' || $order->status=='Confirmed' || $order->status=='Processing' ||$order->status=='Delivered') ? 'completed' : ''}}">
									<span class="is-complete"></span>
									<p>Ordered Placed<br><span>{{date('d-m-Y',strtotime($order->created_at))}}</span></p>
								</div>
								<div class="order-tracking {{($order->status=='Confirmed' || $order->status=='Processing' ||$order->status=='Delivered') ? 'completed' : ''}}">
									<span class="is-complete"></span>
									<p>Ordered Unfullied<br><span>{{$order->status=='Confirmed' ?date('d-m-Y',strtotime($order->updated_at)) : ''}}</span></p>
								</div>
								<div class="order-tracking {{($order->status=='Processing'||$order->status=='Delivered') ? 'completed' : ''}}">
									<span class="is-complete"></span>
									<p>Yet to Dispatched<br><span>{{$order->status=='Processing' ?date('d-m-Y',strtotime($order->updated_at)) : ''}}</span></p>
								</div>
								<div class="order-tracking {{$order->status=='Delivered' ? 'completed' : ''}}">
									<span class="is-complete"></span>
									<p>Dispatched  Order<br><span>{{$order->status=='Delivered' ?date('d-m-Y',strtotime($order->updated_at)) : ''}}</span></p>
								</div>                              
							</div>
                         @else
