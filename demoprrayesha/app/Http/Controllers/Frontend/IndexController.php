    ...
    public function index()
{
 ...
 return response()->json([
    ...
    public function testlogin(){
      return view('frontend.Auth.testauth');
    }
    public function testcreateaccount(){
      return view('frontend.Auth.testcreateaccount');
    }
    public function testguestlogin(){
        return view('frontend.Auth.testguestlogin');
    }
...
// ]);
 $customer = Auth::guard('users')->user() ?? Auth::guard('guest')->user();
        $customer_id = $customer ? $customer->id : null;
          ...
          if ($customer_id) {
        $this->syncCart($customer); // Pass user object (not $user = Auth::user())
    }
  ...
  public function loginsumbit(Request $request)
{
    ...
    */
      ...
      public function Registersumbit(Request $request)
    {
        ...
        $duplicate_check_mail = User::where('email', $request->email)->where('role', 'customer')->first();
        if (isset($duplicate_check_mail)) {
           // Session::put('error', 'Phone number already exists');
            return response()->json(['error' => false, 'message' => 'Mail already exists']);
        }
        ...
        if (Auth::guard('users')->check()) {
           ...
           // Log::info('User logged in successfully', ['user' => $check]);
            ...
            if($aCartData){
                ...
                return response()->json([
                         'error' => false,
                         'authenticated' => Auth::guard('users')->check(), // check if user is logged in
                         'redirect' => route('index'), // or your intended route after login
...
//  $needsWrapping = !isset($rawCart['date']) 
//               || !isset($rawCart['items']) 
//               || (
//                   is_array($rawCart['items']) && empty($rawCart['items']) &&
//                   count(array_filter(array_keys($rawCart), 'is_numeric')) > 0
//               );

// if ($needsWrapping) {
//     // Extract only numeric keys = actual cart items
//     $cartItems = [];
...
//     // If there are cart items, sync to DB
//     if (!empty($sessionCart)) {
//         // Step 1: Clear previous entries
//         CartTable::where('customer_id', $user->id)->delete();
          
//         // Step 2: Save session items to DB
//         foreach ($sessionCart as $item) {
//             CartTable::create([
//                 'customer_id' => $user->id,
//                 'product_id' => $item['product_id'],
//                 'product_varient' => $item['product_varients_id'],
...
//         // Step 3: Clear cart from session if needed
//         session()->forget('cart');
//     } else {
//         // No cart items, just clean up DB cart
//         CartTable::where('customer_id', $user->id)->delete();
//     }
}
