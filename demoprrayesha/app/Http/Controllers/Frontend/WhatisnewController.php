...
$tempChildIdArray = array("155", "165", "177", "181");
    ...
    //else if (in_array($categories->id, $tempChildIdArray)){
        ...
        else if($categories->is_parent == 1 && $categories->parent_id > 0 && $categories->sub_cate_id > 0){
        $products = Product::join('product_variants', 'products.id', '=', 'product_variants.product_id')
        ->where('products.status', 'active')
        ->where('product_variants.status','active')
        ->where('product_variants.in_stock','>','0')
  ...
  if (!empty($size) && is_array($size)) {
    ...
    if (!empty($slug)) {
        if($category->is_parent == 0){ //main category
            $products->whereRaw("find_in_set($category->id,category)");
        }
        elseif($category->is_parent == 1 && $categories->sub_cate_id < 0){ //sub category
            $products->whereRaw("find_in_set($category->id,subcategory_id)");
        }
        elseif($category->is_parent == 1 && $categories->parent_id > 0 && $categories->sub_cate_id > 0){ //child category
...
$products = $products->sortByDesc('current_stock')->values();        
   ...
   public function newarrival(request $request)

    {
        ...
        $products=Product::where('status','active')->where('tag', 'NA')->orderBy('id',"desc")->get();

        $categories=Category::where('is_parent',0)->where('status','active')->get();

     $a_Products = Product::where('status', 'active')
        ->where('tag', 'NA')
        ...
        foreach($a_Products as $product){
            ...
            $category=Category::where('is_parent',0)->where('status','active')
    ...
    $ahover_image_photo=array();
        ...
        foreach($n_Products as $key=>$product){
           ...
           $category=Category::where('is_parent',0)->where('status','active')
        //   ->whereIn('id', explode(',',$product->category))
        //   ->where('home','active')
        ->where('id',$product->category)
           ->first();
            ...
            $highest_rate=Product::where('status','active')->where('tag', 'NA')->max('regular_price');
       ...
       return view('frontend.whatsnew',compact('products','ahover_image_photo','iswishlist','categories','n_Products','aProductSaleprice','aProductvariant_photo','n_ProductSaleprice','n_Productvariant_photo','highest_rate','prod_categories'));
