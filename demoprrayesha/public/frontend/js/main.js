...
//    console.log(path);
  ...
  if (document.getElementById('ship-to-different-address').checked) {
          ...
          if($('#sf_name').val() == "" ||  $('#sphone').val() =="" || $('#semail').val() =="" || $('#sstreet-address').val()=="" ||  $('#stown').val()=="" || $('#spin-code').val()==""){
  ...
  if($('#f_name').val() == "" || $('#phone').val() =="" || $('#email').val() =="" || $('#street-address').val()=="" ||  $('#town').val()=="" || $('#pin-code').val()==""){
      isbillingempty=true;
  }else{
...
$(document).on("click",'.cart_rendercount',function(){
  ...
  if($(this).data('type')=="inc"){
        ...
        $('.product_rate_'+product_id).html(Number(val)+ " x "+ Number(price));
