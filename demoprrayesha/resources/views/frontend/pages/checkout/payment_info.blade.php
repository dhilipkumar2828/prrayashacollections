    ...
    <tbody class="table-body">
       ...
       @foreach($products as $pro)
        ...
        <tr class="cart-item">
            ...
            <td class="product-name">{{$pro->product_name}}<span class="product-quantity"> × {{$pro->product_qty}}</span>

            

           </td>

            <td class="product-total amt_symbol">₹ {{number_format(($pro->price * (int)$pro->product_qty), 2, '.', '')}}
...
?>
<input type="hidden" id="discountvalus" name="discountvalus" value="{{session('coupon')['discount'] ?? 0}}">
    ...
    <td>
        @if(session('coupon'))
            <span class="amount amt_symbol" id="coup">₹{{ number_format(session('coupon')['discount'], 2) }}</span>
        @else
