...
<main>
    ...
    <div class="product-detail">
        ...
        <div class="container">
            ...
            @php
                    ...
                    $variant_val=App\Models\ProductVariant::where('product_id',$products->id)->orderBy('id','desc')->first();
                ...
                <div class="slider-for">
					...
					<div class="product-content">
						<div class="col-lg-5 col-md-6 col-sm-12 col-xs-12 img-content">
      							<img src="{{$photo}}" id="product_img{{$products->id}}"  class="img-responsive" alt="img-holiwood">
      					</div>
                            ...
                            <p class="p1">{!! html_entity_decode ($products->summary) !!}<br class="hidden-md hidden-sm hidden-xs"></p>
                                ...
                                <div class="prince" ><span>$</span><span id="offer_price{{$products->id}}">{{number_format(($variant_value->sale_price))}}</span>
                                <s
                                    class="strike" id="original_price">${{number_format($variant_value->regular_price,2)}}</s></div>
                            ...
                            <div class="size col-md-3 col-sm-6 col-xs-12">
                                <span class="lb-size">{{$key}} <span class="sta-red">*</span></span>
                                <div class="select-custom attrib_colr">
                                <input type="hidden" name="product" id="product{{ $products->id }}" value="{{ $products->id }}"> 
                                @php
                            $arr_cnt=count($data)
                            @endphp
                                <select  name="attrib_{{$key}}_{{ $products->id }}" id="attrib_{{$key}}_{{ $products->id }}" class="productoption attrib_{{ $products->id }}"  data-id="{{$products->id}}">
                                    <option value="">{{$key}}</option>
                                        @foreach($data[$key] as $size)
                                            @foreach(explode(',',$size->arrtibute_value) as $k1 => $size1)
                                            <option value="{{ json_encode($data) .'|'.$products->id.'|'.$key.'|'. $size1 }}" {{ ($k1  == count(explode(',',$size->arrtibute_value))-1) ? "selected" : '' }}>{{ $size1 }}</option>
                                            @endforeach
                                        @endforeach
                                    </select>
                                </div>
    ...
    background: ".$color->arrtibute_name.";
    margin-right: 10px;
    transition: all .3s ease";
                                   @endphp
...
<div class="input-group input-number-group">
    ...
    <div class="input-group-button">
        <span class="input-number-decrement" data-title="newarrivals" data-category="{{$categories->title}}"   data-id="{{$products->id}}">-</span>
    </div>
    <input class="input-number input-numbersnewarrivals_{{$products->id}}" type="number" min="0" max="1000" id="total_quantity" value="01" data-id="{{$products->id}}">
    <div class="input-group-button">
        <span class="input-number-increment" data-title="newarrivals" data-category="{{$categories->title}}"  data-id="{{$products->id}}">+</span>
    </div>
    <span class="dola">₹</span><span class="total-prince_{{$products->id}}"  data-price="{{$variant_value->sale_price }}">{{$variant_value->sale_price }}</span>
</div>

</div>
<div class="add-cart">
<a href="javascript:void(0);" data-quantity="1" data-product-id="{{$products->id}}" class="default_set btn-add-cart add_to_cart add_to_cartid{{$products->id}}" id="add_to_cart{{$categories->title}}_{{$products->id}}">Add to cart</a>
<a href="javascript:void(0);" data-quantity="1" data-product-id="{{$products->id}}" class="default_set btn-add-cart buynow buynowid{{$products->id}}" id="buynow{{$categories->title}}_{{$products->id}}">Buy Now</a>
<a href="javascript:void(0);" class="default_set list-icon icon-2 add_to_wishlist wishlist{{$products->id}} wish1{{$products->id}}" data-quantity="1"
                                    data-id="{{$products->id}}" id="add_to_wishlist_{{$products->id}}">
                                    @php
                                    if(Auth::guard('customer')->check()){
                                           $get_wishlists=DB::table('wishlists')->where('customer_id',auth()->guard('customer')->user()->id)->where('product_id',$products->id)->get();                                         
                                           }else{
                                           $get_wishlists=DB::table('wishlists')->where('product_id',$products->id)->get();
                                           }
                                           @endphp
                                           @if(count($get_wishlists) > 0)
                                            <i class="fa fa-heart whats_new_icon_{{$products->id}} icon1_{{$products->id}}" style="color:red;"></i>
                                            @else
                                            <i class="fa fa-heart whats_new_icon_{{$products->id}} icon1_{{$products->id}}" ></i>
                                            @endif
                                </a>
...
Fusce aliquet, ante cursus gravida sagittis, justo erat rhoncus sapien, eget condimentum ligula magna sed est. Suspendisse molestie ligula tortor. Suspendisse vitae orci ac purus eleifend malesuada at vel tellus. Sed ac semper magna. Mauris consequat blandit risus. Cras dictum eros libero, a scelerisque quam laoreet non. Interdum et malesuada fames ac ante ipsum primis in faucibus. Nullam pharetra euismod felis, a eleifend magna.</p>
                    ...
                    <div id="review" class="tab-pane fade">
                        <div class="reviews_area">
                            ...
                            <ul>
                                <li>
                                    <!-- <h2 class="title">
                                        ...
                                        {{$products->title}}</h2> -->
                                    @php
                                    $reviews=\App\Models\ProductReviews::where(['product_id'=>$products->id,'status'=>'accept'])->latest()->paginate(5);
                                    @endphp
                            ...
                            @else
                                  <p class="py-5"><a href="{{route('user.auth',$products->slug)}}">Click here !</a> you need
                                to login!</p><br>
@endif
                        </div>
                            ...
                            <form method="POST" action="{{route('product.review',$products->slug)}}">
                                ...
                                <div class="col-md-6 psd form-group" style="padding-right: 10px !important;">
                                    ...
                                    <input type="text" class="form-control" name="name" id="name"
                                        placeholder="Your Name" value="{{Auth::guard('customer')->user()->name}}">
                                    @error('name')
                                    <p class="text-danger">{{$message}}</p>
                                    @enderror
                                </div>
                                    ...
                                    <input type="email" class="form-control" name="email" id="email" placeholder="Your EMail" value="{{Auth::guard('customer')->user()->email}}">
                                    @error('email')
                                    <p class="text-danger">{{$message}}</p>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="review" id="comments" rows="5"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      @php 
                                                $products1 =\App\Models\Product::with('rel_prods')->where('slug',$products->slug)->first();
                                                $attribute_get=DB::table('attributes')->get();
                                                $product_orders=\App\Models\ProductAttribute::distinct('arrtibute_name')->where('product_id',$products1->id)->get(['arrtibute_name']);
                                                $attribute_value=\App\Models\ProductAttribute::where('product_id',$products1->id)->orderBy('id','desc')->first();
                                                $product_variant=DB::table('product_variant')->where('product_id',$products1->id)->orderBy('id','desc')->first();
                                                $data=array(); 
                            ...
                            <div class="size col-md-6 col-sm-6 col-xs-12">
                                <span class="lb-size label_name_{{$products->id}}">{{$key}} <span class="sta-red">*</span></span>
                                @php
                            $arr_cnt=count($data)
                            @endphp
                                 ...
                                 <select  name="attrib_{{$key}}_{{ $products->id }}" id="attrib_{{$key}}_{{ $products->id }}" class="productoption attrib_{{ $products->id }} ol-md-6 col-sm-6 col-xs-12" style="text-align:center" data-id="{{$products->id}}" >
                                      <option value="">{{$key}}</option>
                                        @foreach($data[$key] as $size)
                                            @foreach(explode(',',$size->arrtibute_value) as $k1 => $size1)
                                            <option value="{{ json_encode($data) .'|'.$products->id.'|'.$key.'|'. $size1 }}" {{ ($k1  ==  count(explode(',',$size->arrtibute_value))-1) ? "selected" : '' }}>{{$size1 }}</option>
...
<!-- modal-end -->
            <div class="related" style="padding: 40px 0 20px 0!important;">
                <div class="container">
                    ...
                    @php
                @$wishitem=\App\Models\Wishlist::where(['status'=>'active','product_id'=>$data->id,'customer_id'=>auth()->guard('customer')->user()->id])->get();
             @endphp
                        ...
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6 product-category">
                            ...
                            <div class="product-image-category">
                                @php
                                 ...
                                 $product_variant=DB::table('product_variant')->where('product_id',$item->id)->orderBy('id','desc')->first();
                                @endphp
                            
                                <figure class="sale sale{{$item->id}} product-figure"><a href="#"><img src="{{$size_guide[0]}}"
                            ...
                            content:'{{ $item->conditions }}';
                                ...
                                <a href="/product_detail/{{$item->slug}}"><i class="far fa-eye"></i></a>

                                         <a href="javascript:void(0);" class="add_to_wishlist2 wish2{{$item->id}}" data-quantity="1"data-id="{{$item->id}}" id="add_to_wishlist_{{$item->id}}" >
                                          @php
                                          $get_wishlists=DB::table('wishlists')->where('product_id',$item->id)->get();
                                          @endphp
                                           @if(count($get_wishlists) > 0)
                                            <i class="fa fa-heart whats_new_icon_{{$item->id}} icon2_{{$item->id}} " style="color:red;"></i>
                                            @else
                                            <i class="fa fa-heart whats_new_icon_{{$item->id}} icon2_{{$item->id}}" ></i>
                                            @endif
                                  </a>
                            ...
                            <div class="prince">${{number_format($product_variant->sale_price,2)}}<s class="strike">${{number_format($product_variant->regular_price,2)}}</s></div>
                            </div>
