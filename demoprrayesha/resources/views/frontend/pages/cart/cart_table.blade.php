  ...
  @php
    ...
    @endphp
        ...
        @if(auth()->guard('users')->user() || auth()->guard('guest')->user())
                    ...
                    $cart=DB::table('cart_tables')->selectRaw('cart_tables.*,product_variants.in_stock')
                    ->Join('product_variants','product_variants.id','cart_tables.product_varient')->where('customer_id',$id)->get();
            ...
            ?>
                    ...
                    <ul class="aside-cart-product-list" style="{{$stockOutCss}}">
                        ...
                        <li class="aside-product-list-item">
                          ...
                          <a href="#" class="remove cartremove" data-product_id="{{$product->id}}" data-product_varient_id="{{$product_variant->id}}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm4 12H8v-9h2v9zm6 0h-2v-9h2v9zm.618-15L15 2H9L7.382 4H3v2h18V4z"></path></svg> </a>
...
<b style="text-align: center;height: 100vh;display: flex;align-items: center;justify-content: center;">Your Cart is empty{{$cart}} </b>
...
?>
    ...
    <ul class="aside-cart-product-list" style="{{$stockOutCss}}">
        ...
        <li class="aside-product-list-item">
            ...
            <a href="#" class="remove cartremove" data-product_id="{{$session_product[$key]['product_id']}}" data-product_varient_id="{{$key}}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm4 12H8v-9h2v9zm6 0h-2v-9h2v9zm.618-15L15 2H9L7.382 4H3v2h18V4z"></path></svg> </a>
...
<div class="pro-qty">
                            <input type="hidden" id="count_prod_qty{{$key}}" class="count_prod_qty"
                                    value="{{$session_product[$key]['product_qty']}}"
                                    data-product_id="{{$session_product[$key]['product_id']}}"
                                  >

              <input type="text" id="quantity{{$key}}" title="Quantity"
                                    class="quantity count_prod_qty{{$key}}"
                                    data-product_id="{{$session_product[$key]['product_id']}}" 
                               
                                    value="{{$session_product[$key]['product_qty']}}" disabled>
                                    <div class="dec qty-btn cart_rendercount" data-product_price="{{ number_format($session_product[$key]['price'],2,'.','') }}" data-product_id="{{$session_product[$key]['product_id']}}" data-type="dec" data-product_varient_id="{{$key}}" data-quantity="{{$productVariantInStock}}">-</div>
                                    <div class="inc qty-btn cart_rendercount" data-product_price="{{ number_format($session_product[$key]['price'],2,'.','') }}" data-product_id="{{$session_product[$key]['product_id']}}" data-type="inc" data-product_varient_id="{{$key}}" data-quantity="{{$productVariantInStock}}">+</div>
                                </div>
                </div>
