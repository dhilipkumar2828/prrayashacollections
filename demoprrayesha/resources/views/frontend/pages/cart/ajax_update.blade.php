...
$cartDataa = DB::table('cart_tables')->where('customer_id',$customer_id)->where('product_id',$couponProductId)->first();
    if($cartDataa!=''){
...
<div class="cart-totals-wrap">
    ...
    <td>
        @if(session('coupon'))
            <span class="amount amt_symbol">₹{{ number_format(session('coupon')['discount'], 2) }}</span>
        @else
