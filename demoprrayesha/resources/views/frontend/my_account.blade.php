                        ...
                        <div class="col-lg-3 col-md-3">
                            <div class="my-account-tab-menu nav nav-tabs" id="nav-tab" role="tablist">
                                ...
                                <button class="nav-link" id="account-info-tab" data-bs-toggle="tab" data-bs-target="#account-info" type="button" role="tab" aria-controls="account-info" aria-selected="false">Account Details</button>
                                 @endif
                                <button class="nav-link" onclick="window.location.href='{{url('user/logout')}}'" type="button">Logout</button>
                                        ...
                                        <p>Order Id : <strong>{{ $ordersrecently->order_id}} </strong></p>
                                         <p>Order Date : <strong>@php echo date('d-m-Y', strtotime($ordersrecently->created_at)); @endphp </strong></p>
                                          <p>Payment Status : <strong>{!! $sStatus1 !!}</strong></p>
                                           <p>Total : <strong><span class="amt_symbol">₹</span>{{number_format($ordersrecently->sub_total + $ordersrecently->deliver_charge - $ordersrecently->discound_amount- $ordersrecently->ship_discount_amount,2)}} </strong></p>
                                           <?php } ?>
                                    </div>
                                </div>
                                        ...
                                        <div class="myaccount-table table-responsive text-center">
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                                ...
                                                $products=DB::table('products')->where('id',$o->product_id)->first();
                                                 $product_variant=DB::table('product_variants')->where('product_id',$o->product_id)->first();
                                                ...
                                                ?>
                                                    <tr>
                                                            ...
                                                            <td>{{$status22}}</td>
                                                            <td><span class="amt_symbol">₹</span>{{number_format(($o->sub_total + $o->deliver_charge)-($o->discound_amount) - ($o->ship_discount_amount),2)}}</td>
                                                            <td>
                                                                <a class="view_order dropdown-item" href="{{url('/order_pdf/').'/'.$o->id}}">
                                                                  <button type="button" class="default-btn btn-sm">View </button>
                                                                 </a>
                                                                <?php
                                                                 if($o->status!='Received'){
                                                                 ?>
                                ...
                                <!-- <div class="tab-pane fade" id="payment-method" role="tabpanel" aria-labelledby="payment-method-tab">
                                    <div class="myaccount-content">
                                        ...
                                        <div class="account-details-form">
                                            ...
                                            <label for="age1">Union Bank Debit Card</label><br>
                                            <span>**** **** **** 1234</span><br>
                                            <input type="text" placeholder="CVV"><br>
                                            <hr>
                                            <input type="radio" id="age2" name="age" value="60">
                                            <label for="age2">Google Pay UPI</label><br>
                                            <hr>
                                            <input type="radio" id="age3" name="age" value="100">
                                            <label for="age3">Net Banking</label><br>
                                            <hr>
                                            <input type="radio" id="age4" name="age" value="120">
                                            <label for="age4">Cash on Delivery</label><br>
                                        </div>
                                    </div>
                                </div> -->
                                  ...
                                  <div class="myaccount-content addressinfo">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <address>
                                            <h6 class="billname">{{$billing_address->first_name . $billing_address->last_name}}</h6>
                                            <p>{{$billing_address->phone_number}}</p>
                                            <p>{{$billing_address->address}}</p>
                                            <p>{{$state->state}}</p>
                                            <p>{{$billing_address->pincode}}</p>
                                        </address>
                                        <a href="{{url('edit_address/?type=billing')}}" class="editbtn"><i class="fa fa-edit"></i> Edit Address</a>
                                        @endif
                                        ...
                                        <address>
                                        <h6 class="billname">{{$shipping_address->sfirst_name . $shipping_address->slast_name}}</h6>
                                            <p>{{$shipping_address->sphone_number}}</p>
                                            <p>{{$shipping_address->saddress}}</p>
                                            <p>{{$state->state}}</p>
                                            <p>{{$shipping_address->spincode}}</p>
                                        </address>
                                        <a href="{{url('edit_address/?type=shipping')}}" class="editbtn"><i class="fa fa-edit"></i> Edit Address</a>
                                        @endif
                                    </div>
                                        ...
                                        <div class="account-details-form">
                                            <form action="#">
                                                <div class="row">
                                                    <div class="col-lg-6">
                                                        <div class="single-input-item">
                                                            ...
                                                            <input type="text" id="user_name" value="{{$account->name}}"/>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="single-input-item">
                                                    ...
                                                    <input type="email" id="user_email"  value="{{$account->email}}"/>
                                                </div>
                                                <fieldset>
                                                    <legend>Password change</legend>
                                                    <div class="single-input-item">
                                                        <label for="current-pwd" class="required">Current Password</label>
                                                        <input type="password" id="user_previouspassword" name="ltn__name" value="12345">
                                                        <span id="err_prevpassword" style="color:red;display:none;">This Field is Required</span>
                                                        <span id="err_msg" style="color:red;display:none;">Enter correct password</span>
                                                    </div>
                                                    <div class="row">
                                                        <div class="col-lg-6">
                                                            <div class="single-input-item">
                                                                <label for="new-pwd" class="required">New Password</label>                                                              
                                                                <input type="password" id="user_new_password" name="ltn__lastname">
                                                                <span id="err_newpassword" style="color:red;display:none;">This Field is Required</span>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-6">
                                                            <div class="single-input-item">
                                                                <label for="confirm-pwd" class="required">Confirm Password</label>
                                                                <input type="password" id="user_confirm_newpassword" name="ltn__lastname">
                                                                <span id="err_confirmnewpassword" style="color:red;display:none;">This Field is Required</span>
                                                                <span id="err_passwordmatch" style="color:red;display:none;">Password not matches</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </fieldset>
                                                <div class="single-input-item">
                                                    <button class="default-btn" id="update_userdetails" type="button">Save Changes</button>
                                                    <span id="success_msg" style="color:green;display:none;">Details updated successfully</span>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
