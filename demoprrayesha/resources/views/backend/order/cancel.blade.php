...
@include('backend.layouts.notification')
                    <div class="page-content-wrapper ">
                        ...
                        <div class="container-fluid">
                            ...
                            <!-- end row -->
 <div id="success_message" class="alert alert-success" style="display:none;color: black;font-size: 18px;font-family: sans-serif;">Updated Successfully</div>
                                    ...
                                    <div class="card m-b-30">
                                        <div class="card-body">
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                                ...
                                                @php
                                                $order_products=DB::table('order_products')->where('order_id',$item->id)->where('status','Active')->first();
                                                $get_customer=DB::table('customer')->where('id',$item->customer_id)->first();
                                                @endphp
                                                ...
                                                <input type="hidden" name="order_productsid" value="{{$item->id}}">
                                                    <tr>
                                                        ...
                                                        <td>₹ {{ number_format(number_format($item->sub_total,2,'.','') + number_format($item->deliver_charge,2,'.','')   - $item->discound_amount - $item->ship_discount_amount,2) }}</td>
                                                        <td>
                                                            <div class="btn-group m-b-10">
                                                           
                                                           <span class="badge badge-{{ ($item->status != 'Pending') ? (($item->status != 'Confirmed') ? (($item->status != 'Processing') ? (($item->status != 'Delivered') ? (($item->status != 'Returned') ? (($item->status != 'Cancelled') ? '' : 'danger') : 'info') : 'default') : 'warning' ) : 'success' ) : 'warning' }}">{{ $item->status}}</span>
        ...
        <div class="col-sm-6 col-md-12 m-t-30">
                        <div class="modal fade bs-example-modal-center1" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title text-center "><b class="text-center" style="font-size: 18px;font-family: -webkit-body;">Cancel Order</b></h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                            ...
                                            <div class="row">                                             
                                             <div class="col-md-12">
                                                 <div class="form-group row">
                                                     <label for="example-text-input" class="col-sm-6 col-form-label">Order Cancellation Fee </label>
                                                     <div class="col-sm-10">
                                                        <input class="form-control" name="cancellation_fee" id="cancellation_fee" type="text" autocomplete="off" value="0.00" placeholder="Enter Cancellation Fee"   required/>
                                    ...
                                    <div class="modal-button text-center">
                                        <div class="button-items ">
                                        <button type="button" class="btn btn-primary waves-effect" id="cancellation_btn">Cancel Order</button>
