                    ...
                    <div class="page-content-wrapper ">
                        ...
                        <div class="container-fluid">
                            ...
                            <div class="card m-b-30 card-body">
                                <h4 class="card-title font-20 mt-0">Unfullied  Orders</h4>
...
<input type="text" id="search-input" class="form-control" placeholder="Search...">
                                            ...
                                            <table id="example" class="display" cellspacing="0" width="100%">
                                                ...
                                                <tr>
                                                    ...
                                                    <th> Amount</th>
                                                    <th> Status</th>
                                                    @if(auth()->user()->can('processing_orders-view'))
                                                        <th>Actions</th>
                                                    @endif
                                                </tr>
                                                </thead>
                                                            ...
                                                            <div class="btn-group m-b-10">
                                                             <?php
                                                                if($item->status="Processing"){
                                                                    $item->status = "Unfullied Order";
                                                                }else if($item->status="deliver"){
                                                                   $item->status = "Dispatched Order"; 
                                                                }else{
                                                                    $item->status = $item->status;
                                                                }
                                                                ?>
                                                           <span class="badge badge-{{ ($item->status != 'Pending') ? (($item->status != 'Confirmed') ? (($item->status != 'Processing') ? (($item->status != 'Delivered') ? (($item->status != 'Returned') ? (($item->status != 'Canceled') ? '' : 'danger') : 'info') : 'default') : 'warning' ) : 'success' ) : 'warning' }}">{{ $item->status}}</span>
...
$(document).ready(function() {
    $("#search-input").keypress(function(event) {
        if (event.which == 13) {
            // Get the value of a specific input field (optional)
            var inputValue = $('#search-input').val();
             location.href='/progress?search='+inputValue;
